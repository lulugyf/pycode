...
# https://gist.github.com/erogol/7946246   ---- following code come from this web page
...
def rsom(data, cluster_num, alpha, epochs=-1, batch=1, verbose=False):
    ...
    if any([x.op.__class__.__name__ in ['Gemv', 'CGemv', 'Gemm', 'CGemm'] for x in update.maker.fgraph.toposort()]):
        print 'Used the cpu'
    elif any([x.op.__class__.__name__ in ['GpuGemm', 'GpuGemv'] for x in update.maker.fgraph.toposort()]):
        print 'Used the gpu'
    else:
