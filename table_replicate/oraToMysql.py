...
class TableCopy:
    ...
    def copyinit(self):
        ...
        if mode_status:
            #只修改原表数据状态， 不插历史表
            self.sql_select = "select %s from %s where %s='N' order by %s"%(
            ...
            #插历史表
            self.sql_select = "select %s from %s where %s is null order by %s "%(
                        src_clmns, src_tbl, status_clmn, id_clmn)
            ...
            if srctp == 'oracle':
                ...
                self.sql_update_error = "update %s set send_status='E' where %s"%(src_tbl,
                    " and ".join([ "%s=:%s"%(cl,cl) for cl in id_clmn.split(',') ])
                            )
            elif srctp == 'mysql':
                self.sql_delete = "delete from %s where %s"%(src_tbl,
