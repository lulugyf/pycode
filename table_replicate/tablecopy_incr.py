...
class TableCopy:
    ...
    def copyinit(self):
        ...
        try:
            ...
            if self.dbsrc == None:
              self.dbsrc, _ = self.conndb(cfg.get("source", "database1"))
        except Exception,ex:
            logger.debug("connect db fail: %s", self.dbsrc)
        try:
            self.dbdst, _ = self.conndb(cfg.get("target", "database"))
            if self.dbdst == None:
              self.dbdst, _ = self.conndb(cfg.get("target", "database1"))
        except Exception,ex:
            logger.debug("connect db fail: %s", self.dbdst)
        ...
        while 1:
            ...
            try:
                ...
                for r in rows:
                    ...
                    #logger.debug("11rowid: %s read:%s", r1[key_pos[0]], read_flag)
